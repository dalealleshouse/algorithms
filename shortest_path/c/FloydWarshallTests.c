...
static void FloydWarshallShortestPath_Files() {
  ...
  _testFile("src/graphs/10_8.txt", -41, 2, expected);

  const vertex_id expected2[] = {2, 5};
  _testFile("src/graphs/11_8.txt", -89, 2, expected2);

  const vertex_id expected3[] = {1, 12, 15};
  _testFile("src/graphs/14_16.txt", -130, 3, expected3);

  const vertex_id expected4[] = {4, 5, 9, 11, 16};
  _testFile("src/graphs/16_16.txt", -191, 5, expected4);

  const vertex_id expected5[] = {4, 7, 13, 17, 23, 28, 30};
  _testFile("src/graphs/18_32.txt", -355, 7, expected5);

  const vertex_id expected6[] = {2, 20, 21, 26};
  _testFile("src/graphs/19_32.txt", -280, 4, expected6);

  const vertex_id expected7[] = {4,   15,  34,  47,  53,  68,  70,  73,  87,
                                 103, 124, 155, 172, 188, 225, 227, 236, 252};
  _testFile("src/graphs/30_256.txt", -961, 18, expected7);
