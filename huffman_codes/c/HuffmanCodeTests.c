...
static void HuffmanCode_Calculate_SmallCode() {
  ...
  CU_ASSERT_STRING_EQUAL(code->zero->one->zero->zero->symbol->symbol, "B")
  CU_ASSERT_STRING_EQUAL(code->zero->one->zero->one->symbol->symbol, "E")
...
static void HuffmanCode_Calculate_Files() {
  _validateFile("src/data/input_random_9_40.txt", 8, 4);
  _validateFile("src/data/input_random_18_160.txt", 12, 6);
  _validateFile("src/data/input_random_10_40.txt", 9, 4);
