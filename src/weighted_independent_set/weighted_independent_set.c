...
static Result weightedIndependentSet_Init(const size_t n, const int weight,
...
static Result weightedIndependentSetFromVertex(
  ...
  *new_vertices = vertex;
  Result r =
      weightedIndependentSet_Init(1, vertex->weight, new_vertices, result);
...
static Result WeightedIndependentSet_Union(
  ...
  Result result =
      weightedIndependentSet_Init(wis->n + 1, new_weight, vertices, conclusion);
...
static Result wisResursive(WeightedIndependentSet** wis,
  ...
  if (n == 1) return weightedIndependentSetFromVertex(vertices[0], wis);

  WeightedIndependentSet* wis1 = NULL;
  Result result1 = wisResursive(&wis1, vertices, n - 1);
  if (result1 != kSuccess) {
  ...
  Result result2 = wisResursive(&wis2, vertices, n - 2);
  if (result2 != kSuccess) {
    WeightedIndependentSet_Destroy(wis1);
...
static Result dupWeightedIndependentSet(WeightedIndependentSet* input,
...
static void solutionsDestroy(WeightedIndependentSet** solutions,
                             const size_t n) {
  for (size_t i = 0; i < n; i++) WeightedIndependentSet_Destroy(solutions[i]);
}

static Result reconstruct(unsigned long solutions[], PathGraph* graph,
...
Result WeightedIndependentSet_Recursive(PathGraph* graph,
  ...
  return wisResursive(conclusion, graph->vertices, graph->n);
}
...
Result WeightedIndependentSet_Dynamic_Reconstruction(
  ...
  if (graph->n == 1) {
    return weightedIndependentSetFromVertex(graph->vertices[0], conclusion);
  }
  ...
  return reconstruct(solutions, graph, conclusion);
}
...
Result WeightedIndependentSet_Dynamic(PathGraph* graph,
  ...
  Result result = weightedIndependentSetFromVertex(graph->vertices[0], &set1);
  ...
  for (size_t i = 2; i < graph->n + 1; i++) {
    ...
    if (__builtin_add_overflow(iminus2->weight, w->weight, &sum_weight)) {
      solutionsDestroy(solutions, solutions_n);
      return kArithmeticOverflow;
    }
    ...
    if (iminus1->weight >= sum_weight) {
      r = dupWeightedIndependentSet(iminus1, &solutions[i]);
    } else {
  ...
  solutionsDestroy(solutions, solutions_n - 1);
