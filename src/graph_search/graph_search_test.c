...
const static char* small_path = "src/graph_search/test_data/small.txt";
const static char* dir_path = "src/graph_search/test_data/directed.txt";
const static char* scc_path = "src/graph_search/test_data/small-scc.txt";
const static char* simple_scc_path =
    "src/graph_search/test_data/simple-scc.txt";
...
bool conquer(Vertex* x, Vertex* y) {
  ...
  // mark as conquered
  Conqured* c = malloc(sizeof(Conqured));
  c->is_conquered = true;
  x->data = c;

  if (x->id == 5) return false;
...
static void Graph_Reachable_marks_all_reachable() {
  ...
  for (size_t i = 1; i < small_n; i++) {
    if (i == 7) {
      ...
      if (graph->V[i]->data == NULL) {
        CU_FAIL("graph->V[i]->data is NULL");
        return;
      }
      CU_ASSERT_TRUE(((VertexData*)graph->V[i]->data)->am_i_conquered);
    }
...
static void Graph_SCC_MagicOrdering_standard() {
  vertex_id expected[] = {1, 7, 4, 9, 6, 3, 8, 2, 5, 0};
...
static void Graph_SCC_MagicOrdering_standard_two() {
  vertex_id expected[] = {7, 10, 11, 4, 1, 5, 3, 2, 9, 6, 8, 0};
...
int RegisterGraphSearchTests() {
  ...
  CU_TestInfo SCC_tests[] = {
      ...
      CU_TEST_INFO(Graph_SCC_standard),
      CU_TEST_INFO_NULL};
