...
Deprecated_Graph* Deprecated_Graph_Create() {
  return calloc(sizeof(Deprecated_Graph), 1);
}

static size_t _NextAllocSize(unsigned id) {
  if (id < INITIAL_ALLOC) return INITIAL_ALLOC;
...
static void _DeleteEdge(Deprecated_Graph* self, const size_t edge_index) {
...
static void _DeleteVertex(Deprecated_Graph* self, const unsigned id) {
...
Deprecated_Graph* Deprecated_Graph_FromFile(const char* path) {
  ...
  Deprecated_Graph* self = Deprecated_Graph_Create();
  if (self == NULL) {
    fclose(file);
    return NULL;
  }
  ...
  while (fgets(line, BUFFER_SIZE, file)) {
    ...
    while (edge > 0) {
      ...
      if (edge > vertex) Deprecated_Graph_AddEdge(self, vertex, edge);
...
void Graph_Print(const Deprecated_Graph* self, FILE* file) {
  for (size_t i = 0; i < self->n_allocated; i++) {
    if (!self->V[i].initalized) continue;
...
Deprecated_Graph* Graph_Clone(const Deprecated_Graph* source) {
  if (source == NULL) return NULL;

  Deprecated_Graph* clone = Deprecated_Graph_Create();
  ...
  if (source->V != NULL) {
    ...
    if (clone->V != NULL) memcpy(clone->V, source->V, v_size);
  }
  ...
  if (source->E != NULL) {
    ...
    if (clone->E != NULL) memcpy(clone->E, source->E, e_size);
  }

  return clone;
}

void Deprecated_Graph_Destroy(Deprecated_Graph* self) {
  if (self == NULL) return;
